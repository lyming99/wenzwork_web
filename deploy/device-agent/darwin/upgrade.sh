...
agent_require_root
[[ $(agent_host_platform) == darwin ]] || agent_die "this upgrade requires macOS"
...
backup_root=$(agent_validate_absolute_root 'Device Agent backup root' '/Library/Application Support/WenzWork/DeviceAgent/backups')
[[ -n $install_root ]] || install_root=$(agent_installed_root "$config_root/install.conf" /usr/local/lib/wenzwork-device-agent)
...
verifier="$install_root/current/bin/relayctl"
[[ -x $verifier && ! -L $verifier ]] || agent_die "current trusted release verifier is missing"
...
launchctl bootout system/com.wenzwork.device-agent >/dev/null 2>&1 || true
agent_restore_backup "$backup_dir" "$data_root" "$env_file" _wenzworkagent:_wenzworkagent "$backup_root"
...
"$previous_release/scripts/healthcheck.sh" --wait 60 ||
  agent_die "upgrade and rollback both failed; restore $backup_dir manually before re-enabling the service"
